  ...
  return (
    <footer className={styles.footer}>
      <div className={styles.container}>
        ...
        <motion.div className={styles.brand}
          ...
          <div className={styles.brandLogo} onClick={() => navigate("/")}>
            <div className={styles.brandMark}>
              <svg viewBox="0 0 24 24"><path d="M2 20h20M6 20V10l6-6 6 6v10M10 20v-5h4v5" /></svg>
            </div>
            <span className={styles.brandName}>ERP System</span>
          </div>
          <p className={styles.brandDesc}>
            Manage inventory, production, sales, and reports in one place with a simple, structured system.
          </p>
          ...
          <div className={styles.socials}>
            {[FiFacebook, FiTwitter, FiLinkedin, FiGithub].map((Icon, i) => (
              <motion.button key={i} className={styles.socialBtn}
        ...
        <motion.div className={styles.col}
          ...
          {[["Home", "/"], ["About", "/about"], ["Contact", "/contact"]].map(([label, path]) => (
            <button key={label} className={styles.colLink} onClick={() => navigate(path)}>{label}</button>
          ))}
        </motion.div>
        ...
        <motion.div className={styles.col}
          initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.14, ease: [0.22, 1, 0.36, 1] }}>
          <h4 className={styles.colTitle}>Modules</h4>
          {[["Inventory", "/inventory/info"], ["Production", "/production/info"], ["Sales", "/sales"], ["Reports", "/reports/info"]].map(([label, path]) => (
            <button key={label} className={styles.colLink} onClick={() => navigate(path)}>{label}</button>
          ))}
        </motion.div>
        ...
        <motion.div className={styles.col}
          initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.20, ease: [0.22, 1, 0.36, 1] }}>
      ...
      <div className={styles.bottom}>
        <p className={styles.bottomLeft}>© {new Date().getFullYear()} ERP System. All rights reserved.</p>
        <div className={styles.bottomRight}>
          {["Privacy", "Terms", "Security"].map((label) => (
            <button key={label} className={styles.bottomLink}>{label}</button>
          ))}
        </div>
      </div>
    </footer>
