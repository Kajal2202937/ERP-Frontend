...
const Contacts = () => {
  ...
  useEffect(() => {
    ...
    const handleReplyReceiveAdmin = (data) => {
      ...
      setContacts((prev) =>
        prev.map((c) => {
          ...
          if (data.tempId) {
            const hasTempMatch = (c.replies || []).some(
              (r) => r.tempId === data.tempId,
            );
            if (hasTempMatch) {
              return {
                ...c,
                replies: c.replies.map((r) =>
                  r.tempId === data.tempId
                    ? {
                        _id: data._id,
          ...
          const existsById = (c.replies || []).some((r) => r._id === data._id);
          if (existsById) return c;
    ...
    socket.on("contact_reply_receive_admin", handleReplyReceiveAdmin);

    return () => {
      socket.off("contact_reply_receive_admin", handleReplyReceiveAdmin);
    };
  }, []);
  ...
  const sendReply = async () => {
    ...
    try {
      const res = await replyMessage(selectedId, text, tempId);
      const confirmedId = res?.data?._id;
        ...
        prev.map((c) =>
          c._id === selectedId
            ? {
                ...
                replies: c.replies.map((r) =>
                  r.tempId === tempId
                    ? { ...r, _id: confirmedId, _status: "sent" }
                    : r,
                ),
              }
            : c,
  ...
  const markResolved = async () => {
    ...
    try {
      await updateMessage(selectedId, "resolved");
      setContacts((prev) =>
        prev.map((c) =>
